...
import { Weathers } from "./gameboardtile";
...
export type GameTurnInputs =
    ...
    | {
          action:
              | GameActions.GlobalPass
              | GameActions.BosunDetect
              | GameActions.BosunReduceFood
              | GameActions.BosunReducePellets
              | GameActions.GunnerDodge;
      }
    // Coordinate Actions
    | {
          action:
              | GameActions.HelmsmanMove
              | GameActions.TopmanReveal
              | GameActions.GunnerMine;
...
export default class Game implements IGame {
    ...
    constructor(gameId: number) {
        ...
        const yonkaStartColumn: number = Math.floor(
            Math.random() * COLUMN_COUNT
        );
        ...
        this._gameboard = new GameBoard(ROW_COUNT, COLUMN_COUNT, {
            row: yonkaStartRow,
            column: yonkaStartColumn,
        });
        this._yonka = new Yonkadingo({
            row: yonkaStartRow,
            column: yonkaStartColumn,
        });
    ...
    public setClass(socketId: string, classToSet: Classes): boolean {
        ...
        if (
            classToSet === Classes.Bosun &&
            (this._bosun === "" || this._bosun === socketId)
        ) {
            // console.log("Setting user to bosun");
            this._bosun = socketId;
            isUserSetToNewClass = true;
        } else if (
            classToSet === Classes.Gunner &&
            (this._gunner === "" || this._gunner === socketId)
        ) {
            // console.log("Setting user to gunner");
            this._gunner = socketId;
            isUserSetToNewClass = true;
        } else if (
            classToSet === Classes.Helmsman &&
            (this._helmsman === "" || this._helmsman === socketId)
        ) {
            // console.log("Setting user to helmsman");
            this._helmsman = socketId;
            isUserSetToNewClass = true;
        } else if (
            classToSet === Classes.Steward &&
            (this._steward === "" || this._steward === socketId)
        ) {
            // console.log("Setting user to steward");
            this._steward = socketId;
            isUserSetToNewClass = true;
        } else if (
            classToSet === Classes.Topman &&
            (this._topman === "" || this._topman === socketId)
        ) {
    ...
    public removeUserFromGame(socketId: string) {
        if (this._bosun === socketId) {
            this._bosun = "";
            console.log(
                `${rT}${socketId} Left!  Game ${this._id} lost their ${buT}Bosun${rT}!${ansiR}`
            );
        }
        if (this._gunner === socketId) {
            this._gunner = "";
            console.log(
                `${rT}${socketId} Left!  Game ${this._id} lost their ${buT}Gunner${rT}!${ansiR}`
            );
        }
        if (this._helmsman === socketId) {
            this._helmsman = "";
            console.log(
                `${rT}${socketId} Left!  Game ${this._id} lost their ${buT}Helmsman${rT}!${ansiR}`
            );
        }
        if (this._steward === socketId) {
            this._steward = "";
            console.log(
                `${rT}${socketId} Left!  Game ${this._id} lost their ${buT}Steward${rT}!${ansiR}`
            );
        }
        if (this._topman === socketId) {
            this._topman = "";
            console.log(
                `${rT}${socketId} Left!  Game ${this._id} lost their ${buT}Topman${rT}!${ansiR}`
            );
        }
    }
    ...
    public progressTurn() {
        ...
        console.log(
            `turnOrder advanced to ${mT}${Classes[turnOrder[i]]}${ansiR}`
        );
    }
    // #endregion
    ...
    public move(isYonkaMove: boolean, pathToMove: Coordinate[]) {
        ...
        this.gameboard.moveShip(
            isYonkaMove ? this._yonka : this._ai,
            pathToMove
        );
        this.progressTurn();
    }
    ...
    public mine(mineLocations: Coordinate[]) {
        const areLocationsValid = mineLocations.every(
            (mineLocation) =>
                this.yonkadingo.lastTilesMoved.find(
                    (tileMoved) =>
                        mineLocation.column === tileMoved.column &&
                        mineLocation.row === tileMoved.row
                ) !== null
        );
    ...
    public fire(direction: Direction) {
        const yonkaLocation = this.yonkadingo.location;
        let targets: Coordinate[] = [];
        if (direction === "up") {
            for (let i = yonkaLocation.row - 1; i >= 0; i--) {
                targets.push({
                    column: yonkaLocation.column,
                    row: i,
                });
            }
        }
        if (direction === "right") {
            for (
                let i = yonkaLocation.column + 1;
                i < this.gameboard.columnCount;
                i++
            ) {
                targets.push({
                    column: i,
                    row: yonkaLocation.row,
                });
            }
        }
        if (direction === "down") {
            for (
                let i = yonkaLocation.row + 1;
                i < this.gameboard.rowCount;
                i++
            ) {
                targets.push({
                    column: yonkaLocation.column,
        ...
        if (direction === "left") {
            for (let i = yonkaLocation.column - 1; i >= 0; i--) {
                targets.push({
                    column: i,
                    row: yonkaLocation.row,
                });
        ...
        console.debug(
            `${brT}${bwB}FIRING LASER AT${bbB}${mT}`,
            { targets, yonkaLocation, aiLocation: this._ai.location },
            ansiR
        );
        if (
            targets.some((target) => {
                return (
                    target.row === this._ai.location.row &&
                    target.column === this._ai.location.column
                );
            })
        ) {
        ...
        if (
            targets.some((target) => {
                return (
                    target.row === this._yonka.location.row &&
                    target.column === this._yonka.location.column
                );
            })
        ) {
